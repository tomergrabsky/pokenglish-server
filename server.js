...
const DEFAULT_STATE = {
  xp: 0, learnedLetters: [], learnedWords: [],
  quizStreak: 0, bestStreak: 0, totalCorrect: 0, selectedPokemon: 1
};
...
// POST /files - שמור קובץ
app.post('/files', (req, res) => {
  try {
    const { path: filePath, content } = req.body;
    if (!filePath || content === undefined) return res.status(400).json({ ok: false });
...
// GET /export - כל הקבצים ב-JSON אחד
app.get('/export', (req, res) => {
  try {
    const result = {};
    const readAll = (dir, base = '') => {
      if (!fs.existsSync(dir)) return;
      for (const f of fs.readdirSync(dir)) {
        const full = path.join(dir, f);
        const rel = base ? base + '/' + f : f;
        if (fs.statSync(full).isDirectory()) readAll(full, rel);
        else result[rel] = fs.readFileSync(full, 'utf8');
      }
    };
    readAll(FILES_DIR);
    res.json(result);
...
app.listen(PORT, () => console.log(`PokeEnglish server on port ${PORT}`));
